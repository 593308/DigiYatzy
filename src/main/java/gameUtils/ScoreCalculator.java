...
public class ScoreCalculator {
	...
	public ScoreCalculator() {
		...
		Function<List<Integer>, Integer> twoPairs = d -> {
			...
			if (selectedPairValues.size() < 2)
				return 0;
			return (selectedPairValues.get(selectedPairValues.size() - 1)
					+ selectedPairValues.get(selectedPairValues.size() - 2)) * 2;
		};
	...
	public int calculateBonusScore(Player p) {
		if (p.getOnesScore() + p.getTwosScore() + p.getThreesScore() + p.getFoursScore() + p.getFivesScore()
				+ p.getSixesScore() >= 63)
			return 50;
		return 0;
	}
